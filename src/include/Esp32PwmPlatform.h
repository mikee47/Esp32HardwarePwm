...
// Speed mode definitions based on platform capabilities
    ...
    #define ESP32_PWM_AVAILABLE_SPEED_MODES {LEDC_LOW_SPEED_MODE, LEDC_HIGH_SPEED_MODE}
#endif
...
#define ESP32_PWM_VALIDATE_RESOLUTION(res) \
    ((res) >= 1 && (res) <= ESP32_PWM_MAX_RESOLUTION)

#define ESP32_PWM_VALIDATE_CHANNEL(ch) \
    ((ch) >= 0 && (ch) < ESP32_PWM_MAX_CHANNELS)

#define ESP32_PWM_VALIDATE_TIMER(tim) \
    ((tim) >= 0 && (tim) < ESP32_PWM_MAX_TIMERS)
...
#define ESP32_PWM_MAX_FREQ_FOR_RESOLUTION(res, clk_freq) \
    ((clk_freq) / (1U << (res)))
